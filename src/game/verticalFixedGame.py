...
# ---------------------------
# Fixed relative ball drop Y positions per level
# ---------------------------
LEVEL_BALL_Y_POSITIONS = {
...
def run_game(user_name, difficulty, screen_width, screen_height):
    ...
    def reset_ball(trial_index):
        y_positions = LEVEL_BALL_Y_POSITIONS.get(difficulty, [0.0] * max_trials)
        y_percent = y_positions[trial_index] if trial_index < len(y_positions) else 0.0
        x_pos = 0
        y_pos = int(y_percent * screen_height)
        return [x_pos, y_pos], None  # No velocity needed for vertical
    ...
    while running:
        ...
        if not waiting_for_next:
            ...
            if count >= max_trials:
                session_result = ['S' if s['success'] else 'F' for s in game_logs[-max_trials:]]
                ...
                successful_tries = sum(s['success'] for s in game_logs[-max_trials:])
