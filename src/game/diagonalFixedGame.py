...
LEVEL_SPAWN_DATA = {
    1: [(0.1, -2), (0.25, 2), (0.5, -3), (0.75, 3), (0.9, -1),
        (0.2, 2), (0.8, -2), (0.4, 3), (0.6, -3), (0.3, 1)],
    2: [(0.15, 1), (0.3, -1), (0.45, 2), (0.6, -2), (0.75, 3),
        (0.9, -3), (0.1, 1), (0.25, -1), (0.5, 2), (0.7, -2)],
    3: [(0.2, -2), (0.4, 2), (0.6, -3), (0.8, 3), (0.3, -1),
        (0.5, 1), (0.7, -2), (0.9, 2), (0.1, -3), (0.25, 3)],
    4: [(0.15, 2), (0.5, -2), (0.75, 3), (0.1, -3), (0.35, 1),
        (0.6, -1), (0.85, 2), (0.1, -2), (0.9, 3), (0.65, -3)],
    5: [(0.3, -2), (0.9, 2), (0.9, -3), (0.15, 3), (0.45, -1),
        (0.75, 1), (0.1, -2), (0.35, 2), (0.15, -3), (0.9, 3)]
}
...
def run_game(user_name, difficulty, screen_width, screen_height):
    ...
    def reset_ball(trial_index):
        if trial_index < len(LEVEL_SPAWN_DATA.get(difficulty, [])):
            x_percent, x_velocity = LEVEL_SPAWN_DATA[difficulty][trial_index]
        else:
            x_percent, x_velocity = 0.5, 0
        spawn_x = int(x_percent * screen_width)
        spawn_y = 0
        angle_x = x_velocity * difficulty
        angle_y = ball_speed * difficulty
        return [spawn_x, spawn_y], [angle_x, angle_y]

    user_folder = os.path.join("data", user_name)
    os.makedirs(user_folder, exist_ok=True)
    history_file = os.path.join(user_folder, "diagonal_fixed_history.json")
    ...
    ball_pos, ball_vel = reset_ball(0)
    game_logs = []
    success_count = 0
    count = 0
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if waiting_for_next and event.key == pygame.K_RETURN:
                    ...
                    success_count = 0
                    game_logs = []
                    ball_pos, ball_vel = reset_ball(count)
                    cup_x = screen_width // 2 - cup_width // 2
                    waiting_for_next = False
